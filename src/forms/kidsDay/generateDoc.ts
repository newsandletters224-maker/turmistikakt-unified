...
export async function generateDocument(data: FormData): Promise<void> {
  ...
  const yardPlotText = data.hasYardPlot === 'bar'
    ? `Бар${data.yardPlotHectares ? ` (${data.yardPlotHectares} га)` : ''}`
    : data.hasYardPlot === 'joq' ? 'Жоқ' : '_______________';
  sections.push(createParagraph(`${itemNum}. Аулалық учаскенің болуы: ${yardPlotText}`));
  itemNum++;
