...
export async function generateDocument(data: FormData): Promise<void> {
  ...
  children.push(
    new Paragraph({
      ...
      children: [
        ...
        new TextRun({ text: 'Аулалық учаскенің болуы: ', bold: true, size: 22, font: 'Times New Roman' }),
        new TextRun({
          text: data.hasYardPlot === 'бар'
            ? `Бар${data.yardPlotHectares ? ` (${data.yardPlotHectares} га)` : ''}`
            : (data.hasYardPlot || '___________'),
          size: 22,
          font: 'Times New Roman',
        }),
      ],
  ...
  children.push(
    new Paragraph({
      spacing: { after: 100 },
      children: [
        new TextRun({ text: '19. ', size: 22, font: 'Times New Roman' }),
  ...
  const serviceDescriptions = [
    ...
    { key: 'socialCultural', title: 'Әлеуметтік-мәдени қызметтер', desc: 'мерекелер мен демалыс іс-шараларын ұйымдастыру, клуб және үйірме жұмыстарына, мәдени іс-шараларға қатыстыру.' },
    { key: 'socialLabor', title: 'Әлеуметтік-еңбек қызметтері', desc: 'тірек-қимыл аппараты бұзылған балаларға кәсіптік бағдар беру, әлеуметтік дағдылары қалыпты деңгейдегі тұрмыстық және қол шеберлігінің негіздеріне үйрету.' },
  ];
