...
export function generateDocument(data: FormData) {
  ...
  children.push(
    ...
    new Paragraph({
      ...
      children: [
        new TextRun({
          text: `16. Аулалық учаскенің болуы: ${data.hasYardPlot === 'бар' ? `Бар${data.yardPlotHectares ? ` (${data.yardPlotHectares} га)` : ''}` : (data.hasYardPlot || '___')}`,
          font: 'Times New Roman',
          size: 22,
        }),
      ],
  ...
  if (data.psychoneuro) {
    children.push(
      new Paragraph({
        ...
        children: [
          new TextRun({
            text: `17. Психоневрологиялық ауытқулар (созылмалы психикалық аурулар): ${data.psychoneuro}`,
            font: 'Times New Roman',
            size: 22,
          }),
        ],
  ...
  if (data.musculoskeletal) {
    children.push(
      new Paragraph({
        ...
        children: [
          new TextRun({
            text: `18. Тірек-қимыл аппаратының бұзылуы (ТҚА): ${data.musculoskeletal}`,
            font: 'Times New Roman',
            size: 22,
          }),
        ],
  ...
  if (data.somatic) {
    children.push(
      new Paragraph({
        ...
        children: [
          new TextRun({
            text: `19. Соматикалық және өзге де ауыр аурулар: ${data.somatic}`,
            font: 'Times New Roman',
            size: 22,
          }),
        ],
